  ...
  async function handleFiles(fileList) {
    ...
    for (const f of files) {
      try {
        ...
        if (names.indexOf(global.FProp.TARGET_SHEET) >= 0) {
          App.prop.target = { name: f.name, bytes, mtime: f.lastModified || 0 };
          App.prop.report = null; App.prop.blob = null;
        } else if (names.some((nm) => /FES\s+(CSC|NWX|CCS|SKS)\s+BK\s*FCST/i.test(nm))) {
    ...
    const entry = global.FMerge.ingest(App.store, recs, {
      label: names.length === 1 ? names[0] : `${names.length} files`,
